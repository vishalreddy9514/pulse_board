...
/**
 * A copy of producer/src/generator.ts, so the backend can generate its own
 * traffic when it is deployed somewhere that cannot run a second always-on
 * process (a free hosting tier, typically). The standalone producer service
 * remains the real one; this exists so a single-container deployment still has
 * something to stream. Keep the two in step if you change the event mix.
 */
